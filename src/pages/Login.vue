    ...
    <v-row align="center" justify="center" class="fill-height">
      <v-col cols="12" sm="10" md="8" lg="5" xl="4">
        <v-card class="login-card rounded-xl ma-1">
          <v-card-title
            class="text-h4 font-weight-bold text-center text-primary py-2"
          >
            {{ $t("system.name") }}
          </v-card-title>
          <v-card-title
            class="text-h6 font-weight-bold text-center justify-center py-2"
          >
            {{ $t("login.title") }}
          </v-card-title>
            ...
            >
              ...
              <v-text-field
                ...
                :rules="[
                  (v) =>
                    !!v ||
                    $t('rules.required', { field: $t('login.username') }),
                ]"
              ...
              <v-text-field
                ...
                :rules="[
                  (v) =>
                    !!v ||
                    $t('rules.required', { field: $t('login.password') }),
                ]"
...
const savePassword = ref(true); // 新增：記住密碼的狀態，預設勾選
...
onMounted(async () => {
  try {
    const [coursesData, amountData] = await Promise.all([
      api.Auth.classList(),
      api.Auth.loginAmount(),
    ]);
    courses.value = coursesData;
    if (coursesData && coursesData.length > 0) {
      selectedCourse.value = coursesData[0];
    }
    loginAmount.value = amountData;
  } catch (error) {
...
const handleLogin = async () => {
  ...
  try {
    await api.auth.login(username.value, password.value, selectedCourse.value);
...
.login-card {
  ...
  box-shadow: 0px 8px 24px rgba(0, 0, 0, 0.2),
    0px 0px 20px -5px rgba(var(--v-theme-primary), 0.3);
